...
const FeaturedProducts = () => {
  ...
  useEffect(() => {
    const fetchFeaturedProducts = async () => {
      try {
        setLoading(true);
        const featuredProds = await productAPI.getAllProducts({ featured: true });
  ...
  useEffect(() => {
    if (isInView && !isHovering && products.length > 0) {
      const interval = setInterval(nextProject, 4000);
      return () => clearInterval(interval);
    }
    return () => {}; // Cleanup function
  }, [isInView, isHovering, nextProject, products.length]);
  ...
  const getCardAnimationClass = useCallback(
    (index: number) => {
      if (products.length === 0) return "featured-card-hidden";
